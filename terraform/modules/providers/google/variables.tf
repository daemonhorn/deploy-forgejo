...
variable "ssh_public_key" {
  description = "Ed25519 public key material to upload for admin instance access."
  type        = string
}

variable "region" {
  description = "GCP zone (e.g. 'us-east1-b', 'us-central1-a'). Note: GCP uses zones for instances; the enclosing region is derived automatically for regional resources. See: cloud.google.com/compute/docs/regions-zones"
  type        = string
}

variable "plan" {
  description = "GCP machine type (e.g. 'e2-micro' = 2C shared/1 GB, ~$6/mo). See: cloud.google.com/compute/docs/machine-resource"
  type        = string
}

variable "hostname" {
  description = "Instance name / resource name prefix."
  type        = string
}
