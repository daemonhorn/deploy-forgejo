...
### `git clone forgejo:…` fails but the IP URL works

If cloning with the SSH alias (`git clone forgejo:/user/repo.git`) fails while the direct IP URL (`git clone ssh://git@<ip>:2222/user/repo.git`) succeeds, the cause is almost always a stale SSH client config rather than a cert or Forgejo problem.

1. **Stale host key in `known_hosts.forgejo`** — after a weekly rotation the VPS gets a new SSH host key. The `Host forgejo` alias uses `StrictHostKeyChecking yes` and `UserKnownHostsFile ~/.ssh/known_hosts.forgejo`, so the old host key causes "Host key verification failed" before auth even starts. Fix:

   ```bash
   # Remove the stale entry for the old IP, then add the new one
   ssh-keygen -R <old-ip> -f ~/.ssh/known_hosts.forgejo
   ssh-keyscan -p 2222 <new-ip> >> ~/.ssh/known_hosts.forgejo
   ```

2. **Stale `HostName` in `~/.ssh/config`** — the `Host forgejo` stanza generated by `sign-user-key.sh` includes `HostName <ip>`. If the instance was rotated, this IP is outdated. Update it to the current instance IP.

3. **Cert and Forgejo registration are fine** — use `ssh -vvv -F ~/.ssh/config git@forgejo true 2>&1 | head -60` to see exactly where the connection fails. The first `debug1:` line containing `Permission denied` or `Host key verification failed` identifies the layer.

### Checking auth results

On the VPS, every auth attempt is logged via syslog. Check:

```bash
journalctl -t forgejo-auth -n 50
```

| Entry | Meaning |
|---|---|
| `result=ok` | Auth succeeded |
| `result=empty:key_not_registered` | Cert is valid but the base key is not in Forgejo — re-run `sign-user-key.sh` |
| `result=err:extract_failed` | Cert blob corrupt — re-issue with `sign-user-key.sh` |
| no entry at all | sshd rejected before `AuthorizedKeysCommand` ran (cert expired, wrong CA, or stale host key) |

For verbose cert-parsing traces, see the "SSH auth internals" section of `README.md`.
